...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if random:
        ...
        q_dist = q_func[state_1, state_2, :, :]
        max_flat_index = np.argmax(q_dist)
        action_index, object_index = np.unravel_index(max_flat_index, q_dist.shape)
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    ## this section is not proven yet to be true    
    if terminal == True and reward == -0.01:
        q_sample = reward + 0.01
    ...
    ##
        ...
        q_sample = reward + GAMMA * np.max(q_func[next_state_1, next_state_2, :, :])

    old_q = q_func[current_state_1, current_state_2, action_index,
           object_index]
    
    new_q = ALPHA * q_sample + (1 - ALPHA) * old_q
    ...
    while not terminal:
        ...
        if for_training:
            ...
            action_index, object_index =\
                  epsilon_greedy(current_room, current_quest, q_func, epsilon)
