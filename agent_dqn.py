...
def epsilon_greedy(state_vector, epsilon):
    ...
    if random:
        ...
        pred_actions, pred_objects = model(state_vector)
        action_index = torch.argmax(pred_actions)
        object_index = torch.argmax(pred_objects)
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    q_value_action_curr = q_value_cur_state[0][action_index]
    q_value_object_curr = q_value_cur_state[1][object_index]

    # TODO Your code here
    if terminal == True and reward == -0.01:
        q_sample_action = reward + 0.01
        q_sample_object = reward + 0.01
    elif terminal == True and reward == -0.11:
        q_sample_action = reward + 0.01
        q_sample_object = reward + 0.01
    elif terminal == True:
        q_sample_action = reward
        q_sample_object = reward
    else:
        q_sample_action = reward + GAMMA * maxq_next
        q_sample_object = reward + GAMMA * maxq_next


    loss_action = 1/2*(q_sample_action - q_value_action_curr)**2
    loss_object = 1/2*(q_sample_object - q_value_object_curr)**2

    loss = (loss_action + loss_object)/2
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
    ...
    while not terminal:
        ...
        if for_training:
            ...
            action_index, object_index =\
                epsilon_greedy(current_state_vector, epsilon)
        ...
        if not for_training:
            ...
            if n_step == 0:
                ...
                epi_reward += reward * GAMMA ** n_step
            n_step += 1
