...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    if terminal == True and reward == -0.01:
        ...
        q_sample = reward + GAMMA * np.max(np.dot(theta, next_state_vector))
    
    old_q = np.dot(theta, current_state_vector)[index]

    theta[index] = theta[index] + ALPHA * (q_sample - old_q) * current_state_vector
    ...
    while not terminal:
        ...
        if for_training:
            ...
            action_index, object_index =\
                  epsilon_greedy(current_state_vector, theta, epsilon)
